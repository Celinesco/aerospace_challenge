...
export default function ActionRequired({ action }: { action: boolean }) {
    const handleClick = async () => {
        ...
        // left get and make and make an alert
        try {
            const res = await fetch(
                `${process.env.API_URL}${process.env.B_ENDPOINT}`,
                {
                    method: "GET",
                    headers: {
    ...
    if (!action) {
        return (
            ...
            >
                ...
                <Button
                    onClick={handleClick}
                    className="absolute top-2 right-2 dark:bg-blue-600 dark:text-white"
                >
                    Fix all
                </Button>
            </div>
        ...
        return (
            <div className="relative flex items-center justify-center h-24 mb-4 rounded bg-green-100 dark:bg-emerald-500">
                <p className="text-2xl text-gray-900">No action is required</p>
            </div>
        );
    }
}
